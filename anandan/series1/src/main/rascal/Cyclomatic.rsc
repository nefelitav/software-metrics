...
void calculateCyclomaticComplexity(loc projectLoc) {
	...
	visit(asts) {
		case decl: \method(Type _, _, _, _, _): unitComplexities[decl.src] = unitComplexity(decl);
		case decl: \method(Type _, _, _, _): unitComplexities[decl.src] = unitComplexity(decl);
	}
